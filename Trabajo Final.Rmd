...
¿Explica la variacion de precios la densidad y usos de Suelo?
Para ello, trabajaremos los dataset 2019 y 2020 para ver las variaciones 
...
Vamos a hacer que el hexágono tomando como valor el promedio de todos los valores incolucrados.
...
ggplot()+
  ...
  theme_void()
```
En este grafico podemos observar como crece la densidad poblacional hacia los ejes principales. Por otro lado, podemos ver un gradiente en descenso hacia el borde de Gral. Paz. A su vez, en las areas centrales podemos ver reflejado la baja densidad poblacional. 
...
ggplot()+
  ...
  theme_void()
```
En este grafico se evidencia en los ejes principales y en barrios como Recoleta y Palermo mayor diversidad de usos.
...
ggplot()+
  ...
  theme_void()
```
Se puede ver un claro gradiente de precios del Norte a Sur de la Ciudad de Buenos Aires.
...
ggplot()+
  ...
  theme_void()
```
En este grafico se evidencia que la gradiente de precios en periodo 2020 se mantuve en lineas generales.
...
ggplot()+
  ...
  theme_void()
```
Como conclusión podemos ver que los ejes principales y zonas aledañas la variacion de precio fue positiva, como sucedio en el Corredor Norte y Zona Oeste (como en Barrios de Saavedra y Devoto).A su vez, podemos ver que algunas zonas que identificamos con mayor densidad tambien reflejaron una variacion positiva en los precios.
...
ggplot(hexagonos, aes(x=PROM_DENS, y=VARIACION)) +
  ...
  geom_smooth(method=lm,  linetype="dashed",
             color="darkred")+
  labs (title = "Correlacion entre la Densidad y Variacion",
       subtitle = "Ciudad Autónoma de Buenos Aires, 2019-2020",
       caption ="Fuente: GCBA data", 
...
Se evidencia una leve correlacion positiva entre la densidad y variacion de precios de lotes. Por otro lado aparente que en zonas menos densas la variabilidad fue mayor(mayor rango de variacion de precios).
...
Si bien la pendiente es positiva No podemos asegurar que haya una correlacion feaciente entre la diversidad y la variacion de precios.Pero si Notamos que hay una alta variabilidad a poca diversidad de usos.
