...
```{r}
hex2 <- select(hex2, LATITUD, LONGITUD, M2TOTAL, PRECIOUSD, geometry, index)%>%
group_by(index)%>%
...
ggplot()+
  geom_sf(data=hex, fill="grey85") +
  geom_sf(data=hex2, aes(color=index))+
  scale_color_viridis_c()+
  theme_void()
```
```{r}
...
```{r}
hex_promedio_2019<- left_join(hex3, hex, by="index") #union espacial con la grilla original
```
...
```{r}
hex2 <- select(hex2, geometry,  PRECIOUSD, BARRIO, COMUNA_,index)%>%
group_by(index)%>%
summarise (PROMEDIOM2 = mean(PRECIOUSD))
```
