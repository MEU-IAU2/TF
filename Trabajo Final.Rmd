...
```{r}
Terrenos_2019_seleccion <- select(Terrenos_2019, LATITUD, LONGITUD, M2TOTAL, PRECIOUSD, BARRIOS, COMUNA, geometry) 
  
```

```{r}
hex4 <- Terrenos_2019_seleccion %>% st_join(hex)
```

```{r}
hex5 <- select(hex4, LATITUD, LONGITUD, M2TOTAL, PRECIOUSD, geometry, index)%>%
group_by(index)%>%
summarise (PROMEDIOM2 = mean(PRECIOUSD))
```

```{r}
ggplot()+
  geom_sf(data=hex, fill="grey85") +
  geom_sf(data=hex5, aes(color=index))+
  scale_color_viridis_c()+
  theme_void()
```
```{r}
...
```{r}
hex_promedio_2019<- left_join(hex6, hex, by="index") #union espacial con la grilla original
```

```{r}
ggplot()+
  geom_sf(data=hex_promedio_2019, aes(fill=PROMEDIOM2, geometry=grid)) +
...
Para facilitar el trabajo en equipo, este set de datos fue procesado en TERRENOS 2020.Rmd, dentro del mismo repositorio. Resume la venta de terrenos en CABA en el año 2020
...
```{r}
Terrenos_2020_seleccion <- select(Terrenos_2020, PRECIOUSD, BARRIO, COMUNA_, geometry) 
```

```{r}
hex6 <- Terrenos_2020_seleccion %>% st_join(hex)
```

```{r}
hex7 <- select(hex6, geometry,  PRECIOUSD, BARRIO, COMUNA_,index)%>%
group_by(index)%>%
summarise (PROMEDIOM2 = mean(PRECIOUSD))
```
...
ggplot()+
  geom_sf(data=hex, fill="grey85") +
  geom_sf(data=hex7, aes(color=index))+
  scale_color_viridis_c()+
  theme_void()
```
...
```{r}
hex_promedio_2020<- left_join(hex8, hex, by="index") #union espacial con la grilla original
```

```{r}
ggplot()+
  geom_sf(data=hex_promedio_2020, aes(fill=PROMEDIOM2, geometry=grid)) +
  geom_sf(data=barrios_CABA, fill="NA")+
  scale_fill_viridis_c() +
  theme_void()
```

##COMPARAR AMBOS PROMEDIOS POR HEXÁGONO VIENDO CUANTO VARIO CADA UNO, A FIN DE IDENTIFICAR LOS QUE MAS VARIARON Y DONDE SE ENCUENTRAN.
