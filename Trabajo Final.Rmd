...
```{r}
ggplot()+
  geom_sf(data=Terrenos_2019_seleccion, aes(fill=BARRIOS), color=NA)+
  geom_sf(data = barrios_CABA, fill=NA, size=.8) + 
  geom_sf(data=hex4, fill=NA, color="white")+
  theme_void()
```
...
Ahora agruparemos “Calle” y “VALORXM2”

agrupar <- group_by(departamentos_en_venta, CALLE)
resumir <- summarise(agrupar, U_S_M2=mean(U_S_M2))
head(resumir)
...
```{r}
ggplot()+
  geom_sf(data=Terrenos_2019, aes(fill=PRECIOUSD), color=NA)+
  geom_sf(data = barrios_CABA, fill=NA, size=.8) + 
  geom_sf(data=hex, fill=NA, color="white")+
  scale_fill_viridis_c()+
  theme_void()
